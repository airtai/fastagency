...
uvicorn my_fastagency_app_without_auth.deployment.main_1_nats:app --host 0.0.0.0 --port $NATS_FASTAPI_PORT > /dev/stdout 2>&1 &

# Run uvicorn server
uvicorn my_fastagency_app_without_auth.deployment.main_2_fastapi:app --host 0.0.0.0 --port $FASTAPI_PORT > /dev/stdout 2>&1 &
...
do
    ...
    gunicorn --workers=1 my_fastagency_app_without_auth.deployment.main_3_mesop:app --bind 0.0.0.0:$PORT > /dev/stdout 2>&1 &
done
