...
# Run uvicorn server
uvicorn my_fastagency_app_without_auth.deployment.main_:app --host 0.0.0.0 --port $FASTAPI_PORT > /dev/stdout 2>&1 &
...
do
    ...
    gunicorn --workers=1 my_fastagency_app_without_auth.deployment.main:app --bind 0.0.0.0:$PORT > /dev/stdout 2>&1 &
done
