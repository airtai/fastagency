...
class AutoGenWorkflows(WorkflowsProtocol):
    def __init__(self) -> None:
        ...
        self._workflows: dict[str, tuple[Callable[[UI, UUID], str], str]] = {}
    ...
    def run(
        ...
        workflow_uuid = workflow_uuid or uuid4()
        workflow, description = self._workflows[name]

        iostream = IOStreamAdapter(ui)

        with IOStream.set_default(iostream):
            return workflow(ui, workflow_uuid, **kwargs)  # type: ignore[call-arg]
